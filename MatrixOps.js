...
function matVecMultSparse(matrix, vector) {
	...
	for(var i = 0; i<size; i++) {
		...
		for(var j = 0, k = 0; j<size; j++) {
			if(matrix[i][j]=0 || vector[k] != j ) {
				result[i][j] = 0;
			}
			else {
			result[i][j] = matrix[i][j]*vector[k+1];
			k++;
			}	
		}
	}
...
function compressHigh(matrix, quality) {
	...
	// depp copy matrix
	for(var i = 0; i<rows; i++) {
		compressed[i] = Int16Array.from(matrix[i]);
	}
	
	
	
	
	var rows = compressed.length;
	
	// last row = splitSize!!
	var split = (rows-1)/2;
	
	var rowSplit = compressed[0].length/2
	...
	for(var i = 0; i<split; i++) {
		...
		var secondHalf = createSparseVector(compressed[i], rowSplit, threshold);
		
		
		// first half only zeroes => store only second half
		if(compressed[i].subarray(0, rowSplit).every(isZero)){
			compressed[i] = secondHalf;
		}
		...
		newRowVector.set(compressed[i].slice(0, rowSplit));
		newRowVector.set(secondHalf, rowSplit);
		compressed[i] = newRowVector;
		}
	}
	
	// compress both lower quadrants of the matrix
	for(var i = split; i<rows-1; i++ ) {	
		
		
		
		var newRowVector = createSparseVector(compressed[i], 0, threshold);
		compressed[i] = newRowVector;
	}
	return compressed;
}
...
function compressMatrix(matrix, quality) {
	...
	var zerocount = 0;
		...
		for(var j = 0; j < matrix[i].length; j++) {
			...
			// don't compress padded splitSize!
...
/**
 * Decompresses a matrix 
 * 
 * 1) create matrix with zeros
 * 2) fill in non-zero-values
 * 
 * (note: ca. 10 times faster)
 * 
 * @param matrix matrix to be decompressed
 * @param rows rows of the original matrix
 * @param cols columns of the original matrix
 */


function decompressMatrixFill(matrix, rows, cols) {
	
	
	
	var decompressed = numeric.rep([rows, cols ],0);
	...
	for(var i = 0; i < rows; i++) {
		for(var j = 0; j< cols; j++) {
			if(matrix[i][j] <= 25000) {
				decompressed[i][j] = matrix[i][j]
			
			
			// grouped zeros: skip in matrix
			} else{
				j += (matrix[i][j] - 25000)
			}
		}
